...
io.on('connection', (socket) => {
    ...
    socket.on('setGame', (data) => {
        ...
        // Fisher-Yates シャッフル
        [rooms[rid].mainDeck, rooms[rid].cheerDeck].forEach(d => {
        ...
        // ライフの自動配置 (推しのHP分、エールデッキから裏向き・横向きで配置)
        const lifeCount = data.oshi.hp || 0;
        for (let i = 0; i < lifeCount; i++) {
            if (rooms[rid].cheerDeck.length > 0) {
                const c = rooms[rid].cheerDeck.pop();
                rooms[rid].fieldState[c.id] = {
                    ...c,
                    zoneId: 'life-zone',
                    isFaceUp: false,
                    isRotated: true,
        ...
        // ターン管理の初期化
        // 最初のプレイヤーは部屋の作成者(players[0])とする
        rooms[rid].turnState = {
            turnCount: 1,
            currentPlayer: rooms[rid].players[0],
            phase: 'reset' // reset -> draw -> cheer -> main -> arts -> end
        };

        io.to(rid).emit('gameStarted', {
            fieldState: rooms[rid].fieldState,
            deckCount: { main: rooms[rid].mainDeck.length, cheer: rooms[rid].cheerDeck.length },
            turnState: rooms[rid].turnState
        });
    });
    ...
    socket.on('attachAyle', ({ cheerId, targetId }) => {
        ...
        // 座標を対象の少し下にずらして配置し、zIndexを調整
        // データ構造としては、targetIdを親として持つか、あるいは単純に追従させるか。
        // ここでは簡易的に「対象と同じゾーン」「対象の少し下」に移動させるだけにする (Phase 1)
        // 将来的には attachedTo プロパティで管理推奨

        cheer.zoneId = target.zoneId;
        cheer.attachedTo = targetId; // サーバー側で紐づけ保存

        // 通常の移動イベントとして全体に通知（表示位置の調整はクライアント側で行われる想定だが、一旦座標は同期しない）
        // クライアント側で紐づけを知る必要があるため、更新データを返す
        io.to(socket.roomId).emit('cardAttached', { cheerId, targetId, zoneId: target.zoneId });
    });
