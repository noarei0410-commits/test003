...
function openZoom(cardData, cardElement = null) {
    // 裏向きカードの場合は裏面のまま表示（詳細は見せない）
    const isFaceDown = cardElement && cardElement.classList.contains('face-down');
    if (isFaceDown) {
        // 裏面ズーム（必要であれば専用の画像などを表示）
        // 現状は拡大しても何もしない、あるいはシンプルな裏面を表示するだけに留める
        // ユーザー要望「裏面の状態ではカードを拡大しても裏面だけが表示されるようにして」
        // ここでは裏面の拡大表示を実装する
        ...
        zoomOuter.style.backgroundColor = '#16213e'; // 裏面色
    ...
    zoomOuter.style.border = ''; // インラインスタイルリセット
    zoomOuter.style.backgroundColor = 'white';
    ...
    if (cardData.type === 'support') {
            ...
            <div class="skill-box">
                <div class="skill-top-row">
                    <div class="skill-label-container">
                        ${typeLabel ? `<div class="effect-label ${typeClass}">${typeLabel}</div>` :
                    (s.cost || []).map(c => `<div class="cost-dot-small" style="background:${COLORS[c] || 'white'}"></div>`).join('')}
                    </div>
        ...
        // バトンタッチコストのアイコン化（コスト分の色付きアイコンを表示）
        // 指定がない場合は無色(グレー)など、あるいは全体色に合わせる
        // ここではコスト数分だけ、カードの属性色（あるいは汎用エネルギーアイコン）を表示する
        const batonColor = COLORS[cardData.color] || '#95a5a6';
        const batonDots = Array(cardData.baton || 0).fill(`<div class="baton-dot-large" style="background:${batonColor}"></div>`).join('');
        ...
        contentInner.innerHTML = `
            ...
            <div class="zoom-bottom-left-group">
                ...
                <div class="zoom-baton-row">
                    <span>バトンタッチ:</span>
                    <div class="baton-icon-list">${batonDots}</div>
                </div>
                ${extraHtml}
            </div>`;
    }
