...
function createCardElement(data, withEvents = true) {
    ...
    nameSpan.innerText = data.name || "None"; el.appendChild(nameSpan);
    ...
    if (data.type === 'holomen' || data.type === 'oshi') {
        ...
        if (data.color) {
            ...
            clDiv.className = `card-color-icon color-${data.color.toLowerCase()}`; 
