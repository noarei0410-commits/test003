...
function openZoom(cardData, cardElement = null) {
    ...
    const renderContent = () => {
        ...
        // メインパネル描画
        const mainPanel = document.getElementById('zoom-main-panel');
        // スタック情報(stackCards)を渡して、コスト判定を行えるようにする
...
function generateCardDetailHtml(cardData) {
    ...
    if (cardData.type === 'support') {
        ...
        // LIMITEDがある場合、名前バーの角丸を調整するためにクラスを変える等の工夫ができるが、CSS単純化のためそのまま
        const nameRadiusStyle = cardData.limited ? "border-radius: 0 0 8px 8px;" : "border-radius: 8px;";

        return `
            <div class="holomen-zoom-layout" style="border: 5px solid rgba(188,226,232,1); padding: 20px; display:block;">
                <!-- 左上ヘッダー群 -->
                <div class="zoom-support-header-group">
                    <div class="zoom-support-label">サポート</div>
                    <div class="zoom-support-category-tab">${cardData.category || 'Event'}</div>
                </div>

                <!-- 中央積み上げ (LIMITED -> 名前 -> テキスト) -->
                <div style="margin-bottom: auto;">
                    ${limitedHtml}
                    <div class="zoom-support-name-bar" style="${nameRadiusStyle}">
                        ${cardData.name}
                    </div>
                    
                    <div class="zoom-support-text-box">
                        ${cardData.text}
                    </div>
                </div>

                <!-- フッター (タグ) -->
                <div class="zoom-footer-row" style="margin-top: 20px;">
                    <div class="zoom-bottom-left-group">
                         <div class="zoom-tags-row" style="font-size:14px; font-weight:bold; color:#3498db;">#${cardData.tags ? cardData.tags.join(' #') : ''}</div>
                    </div>
                </div>
            </div>`;
    }
        ...
        const skillsHtml = (cardData.skills || []).map(s => {
            ...
            // If it's a special effect, Label is on left, Name is centered.
            // If it's a normal Art, Cost is on Left, Name is centered (or left + damage right).
            // User request: "Artsに必要なエールをアイコンで左側に表示" -> So Cost is Left.
            // "文字がかぶらないように" was previous request.

            let topRowContent = "";

            if (typeLabel) {
                // Effect Layout: Label(Left) - Name(Center) - No Damage
                topRowContent = `
                ...
                // Normal Art Layout: Cost(Left) - Name(Center) - Damage(Right)
                // NOTE: User asked for "Cost icons on left".
                // Previous layout was Name Left, Damage Right.
                // Now we want Cost Left.  Where does Name go?
                // "アーツ名と同じく真ん中に配置して" (from previous request about effects matching arts name center?) -> implicit that Arts Name should be center too?
                // Usually Arts Name is Center or Left. Let's try Center to match the Effects style for consistency, 
                // OR Left (after Cost) if standard TCG style.
                // Given "文字がかぶらないように", Center Absolute is risky if Cost is wide.
                // Let's go with: Cost (Left) ... Name (Center Absolute) ... Damage (Right).
...
/**
 * アーツ発動判定
 * @param {Array} costArray ['青', 'any', ...]
 * @param {Array} stackCards 
 */
function checkCostSatisfied(costArray, stackCards) {
    ...
    stackCards.forEach(c => {
        if (c.type === 'ayle') {
            // colorプロパティがあればそれ、なければ名前の頭文字
            let color = c.color;
            if (!color && c.name) color = c.name.charAt(0);
    ...
    // 2. 無色コスト(any)の消費
    const anyNeeded = costArray.filter(c => c === 'any').length;
    if (pool.length < anyNeeded) {
        return false; // 数が足りない
    }
 ...
 * アーツ発動処理
 */
window.activateArt = (cardId, artName) => {
    // サーバーに通知、またはエフェクト表示
    console.log(`Activate Art: ${artName} from Card ${cardId}`);
    // Zoomを閉じる？ そのまま？
    // socket.emit('useArt', ...); 
    alert(`「${artName}」を発動しました！`);
};
...
function checkCostSatisfied(costArray, stackCards) {
    ...
    // エールカードのみ抽出して色を取得
    // エールの色はとりあえず name の一文字目とする (本来なら別プロパティ推奨)
    // game-logic.js内のcreateCardElement等での色判定ロジックに準拠
    // "青エール" -> "青"

    // 現在のスタックにあるエール色のリストを作成
    let availableColors = [];
    stackCards.forEach(c => {
        if (c.type === 'ayle') {
            // colorプロパティがあればそれ、なければ名前の頭文字
