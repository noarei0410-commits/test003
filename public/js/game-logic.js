    ...
    const renderContent = () => {
        ...
        if (hasStack) {
            ...
            document.querySelector('.stack-header').innerHTML = `
                ...
                <div style="font-size:12px; font-weight:normal;">(Holomen: ${holoCount}, Cheer: ${cheherCount})</div>
            `;
...
function generateCardDetailHtml(cardData) {
    ...
    if (cardData.type === 'support') {
        ...
        const skillsHtml = (cardData.skills || []).map(s => {
            ...
            if (s.type === 'gift') { typeLabel = "Gift"; typeClass = "label-gift"; damageHtml = ""; } // ラベルを英語表記に統一（画像に合わせる場合）
