        ...
        const skillsHtml = (cardData.skills || []).map(s => {
            ...
            try {
                if (s.type === 'arts' || (!s.type && s.damage)) {
                    if (canActivate) {
                        // Store skill data globally for access from onclick
                        const skillKey = `${cardData.id}_${s.name}`.replace(/\s+/g, '_');
                        if (!window.artsSkillData) window.artsSkillData = {};
                        window.artsSkillData[skillKey] = { cardData, skill: s };

                        buttonHtml = `<button class="btn-art-active" onclick="event.stopPropagation(); activateArt('${cardData.id}', '${s.name}', window.artsSkillData['${skillKey}'].cardData, window.artsSkillData['${skillKey}'].skill)">発動</button>`;
                    } else {
