        ...
        const skillsHtml = (cardData.skills || []).map(s => {
            ...
            if (s.type === 'arts' || (!s.type && s.damage)) { // Normal Art assumption
                if (canActivate) {
                    ...
                    const skillKey = `${cardData.id}_${s.name}`;
                    if (!window.artsSkillData) window.artsSkillData = {};
                    window.artsSkillData[skillKey] = { cardData, skill: s };

                    buttonHtml = `<button class="btn-art-active" onclick="activateArt('${cardData.id}', '${s.name}', window.artsSkillData['${skillKey}'].cardData, window.artsSkillData['${skillKey}'].skill)">発動</button>`;
                } else {
