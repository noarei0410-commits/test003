    ...
    if (cardData.type === 'holomen') {
        const skillsHtml = (cardData.skills || []).map(s => {
            ...
            if (s.cost && Array.isArray(s.cost)) {
                ...
                // Cost Visuals only here
                // Note: Activation check is deferred to the button generation block to use ArtsManager
            } else {
                canActivate = true; // No cost = free
            }
            ...
            let buttonHtml = "";
            try {
                if (s.type === 'arts' || (!s.type && s.damage)) {
                    // Use ArtsManager for Check
                    if (window.artsManager && window.artsManager.canActivate(s, stackCards)) {
                        canActivate = true;
                        // Register to get ID
                        const btnId = window.artsManager.register(cardData, s, stackCards);

                        buttonHtml = `<button class="btn-art-active" onclick="event.stopPropagation(); window.activateArtById('${btnId}')">発動</button>`;
                    } else {
                        // Fallback check if manager missing
                        if (!window.artsManager && typeof checkCostSatisfied === 'function' && checkCostSatisfied(s.cost, stackCards)) {
                            // Legacy/Fallback path (should not happen if loaded correct)
                            canActivate = true;
                        }

                        if (canActivate) {
                            // This path only hits if manager missing but utils present
                            buttonHtml = `<button class="btn-art-inactive">Manager Missing</button>`;
                        } else {
                            buttonHtml = `<button class="btn-art-inactive" onclick="event.stopPropagation();">不足</button>`;
                        }
                    }
                }
            } catch (err) {
                console.error("Error generating art button:", err);
                buttonHtml = `<button class="btn-art-inactive">Error</button>`;
            }
            ...
            return `<div class="skill-box ${canActivate ? 'art-active' : ''}">
