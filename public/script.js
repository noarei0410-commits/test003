...
const modal = document.getElementById('setup-modal'), field = document.getElementById('field'), handDiv = document.getElementById('hand');
...
socket.on('init', (d) => { field.querySelectorAll('.card').forEach(c => c.remove()); for (const id in d.fieldState) restoreCard(id, d.fieldState[id]); });
...
document.getElementById('main-deck-zone').onpointerdown = (e) => { e.preventDefault(); socket.emit('drawMainCard'); };
document.getElementById('cheer-deck-zone').onpointerdown = (e) => { e.preventDefault(); socket.emit('drawCheerCard'); };
